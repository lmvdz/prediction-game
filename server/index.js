...
import { init } from 'sdk/lib/admin';
...
;(async () => {
    ...
    init(owner, connection, process.env.CLUSTER, mint)
})();
        ...
        if (retry < 10) {
            try {
                ...
                if (account.isInitialized) {
                    mintTo(connection, owner, mintKeypair.publicKey, address, owner, BigInt(((new BN(1000)).mul((new BN(10)).pow(new BN(mintDecimals)))).toString()), [owner]).then((signature) => {
